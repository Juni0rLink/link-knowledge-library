  ...
  if (provider === 'claude') {
    ...
    fetch('https://api.anthropic.com/v1/messages', {
      ...
      if(d.error && (d.error.message||'').toLowerCase().includes('model') && d._status !== 401 && d._status !== 403) {
        ...
        localStorage.setItem('claude-working-model', workingModel);
        cb((d.content&&d.content[0]&&d.content[0].text)||'');
      }
...
function renderPersonalModulesGrid() {
  ...
  grid.innerHTML = personalModules.map(function(m, i){
    return '<div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:16px;cursor:pointer;transition:box-shadow .15s" '
      ...
      +'<div style="font-weight:700;font-size:13px;margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+escHtml(m.name)+'">'+escHtml(m.name)+'</div>'
...
function openPersonalModule(mi) {
  ...
  function buildSubGrid() {
    ...
    return m.items.map(function(item, si) {
      return '<div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:16px;cursor:pointer;transition:box-shadow .15s" '
        ...
        +'<div style="font-weight:700;font-size:13px;margin-bottom:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+escHtml(item.name)+'">'+escHtml(item.name)+'</div>'
  ...
  d.innerHTML =
    ...
    +'<span style="color:#fff;font-size:16px;font-weight:700;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+escHtml(m.name)+'">'+escHtml(m.name)+'</span>'
